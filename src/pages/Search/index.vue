    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            <li class="with-x" v-if="searchParams.categoryName">{{searchParams.categoryName}}<i @click="removeCatagoryName">x</i></li>
            <li class="with-x" v-if="searchParams.keyword">{{searchParams.keyword}}<i @click="removeKeyword">x</i></li>
             <li class="with-x" v-show="searchParams.trademark"> {{ searchParams.trademark.split(":")[1]}}<i @click="removeTradeMark">×</i>
            </li>
              <li
              class="with-x" v-for="(attrValue, index) in searchParams.props" :key="index">{{ attrValue.split(":")[1] }}<i @click="removeAttr(index)">×</i>
            </li>
          </ul>
        </div>
  ...
  export default {
    ...
    return {
      ...
      searchParams: {
        ...
        props: [], //商品属性的搜索条件
        trademark: "", //品牌的搜索条件
        order: "1:dasc", //排序的参数 【默认初始值:1:desc】
        pageNo: 1, //当前分页器的页码  【默认初始值:1】
        pageSize: 9, //代表当前一页显示几条数据 【默认初始值:10】
      },
    };
  },
  beforeMount(){
